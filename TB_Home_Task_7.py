            ...
            if self.flag:
                if len(self.flag) == 1:
                    if self.flag.isdigit():
                        if int(self.flag) <= 4:
                            if self.flag == '4':
                                wffi = WriteFromFileInterface(self.publish_date, self.my_file)
...
class WriteFromFileInterface(Interface):
    def __init__(self, publish_date, my_file):
        super().__init__()
        self.flag = ''
        self.disk = ''
        self.folder = ''
        ...
        self.publish_date = publish_date
        self.my_file = my_file
        self.def_path = r'c:\ccc\template_TB.txt'
                ...
                if len(self.flag) == 1:
                    if self.flag.isdigit():
                        if int(self.flag) <= 3:
                            ...
                            if self.path:
                                wff = WriteFromFile(self.path, self.publish_date, self.my_file)
                                wff.write_from_file()
                        else:
...
class WriteFromFile(WriteFromFileInterface):
    def __init__(self, path, publish_date, my_file):
        super().__init__(publish_date, my_file)
        self.rec_flag = None
        self.err_path = ''
        self.part_list = []
        self.paragraph = ''
